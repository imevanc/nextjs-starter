...
test("has all the accessible elements", async ({ page }) => {
  ...
  await expect(
      page.getByText('Lorem ipsum dolor sit amet, consectetur adipiscing elit. Curabitur eleifend tempor nisi, eget suscipit ante elementum ac. Praesent euismod arcu lacus, sit amet mollis libero egestas vel.')
  ).toBeVisible();
  await expect(
  ...
  await expect(
      ...
      page.getByTestId('Product-desktop')
  ).toBeVisible();await expect(
      page.getByTestId('Features-desktop')
  ).toBeVisible();await expect(
      page.getByTestId('Marketplace-desktop')
  ).toBeVisible();await expect(
      page.getByTestId('Company-desktop')
  ).toBeVisible();
});
